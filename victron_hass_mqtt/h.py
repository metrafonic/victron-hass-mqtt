...
class Sensor(Component):
    def __init__(
        ...
        self.name = f"{self.parent_device.name} {name}"
        self.object_id = name.replace(" ", "_").lower()
    ...
    def send(self, value=None, blocking=False):
        if not value:
            ...
            publish_value = value
        if self.multiplier:
            publish_value = round(float(publish_value) * self.multiplier, 2)
        if self.mov_avg:
            self.last_n.append(float(publish_value))
            publish_value = round(float(round(sum(self.last_n),2) / len(self.last_n)),2)
